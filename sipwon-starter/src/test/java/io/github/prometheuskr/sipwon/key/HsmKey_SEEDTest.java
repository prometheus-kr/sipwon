...
class HsmKey_SEEDTest {
    ...
    void encrypt_decrypt() throws Exception {
        try (HsmSession session = hsmSessionFactory.getHsmSession(tokenLabel)) {
            HsmKey_SEED hsmKey = (HsmKey_SEED) session.findHsmKey(keyLabel, HsmKeyType.SEED);
            String encrypted = hsmKey.encrypt(PLAIN_STRING_FOR_ENCRYPT, HsmMechanism.SEED_CBC);
            assertThat(encrypted).isNotNull();

            String decrypted = hsmKey.decrypt(encrypted, HsmMechanism.SEED_CBC);
    ...
    void mac() throws Exception {
        try (HsmSession session = hsmSessionFactory.getHsmSession(tokenLabel)) {
            HsmKey_SEED hsmKey = (HsmKey_SEED) session.findHsmKey(keyLabel, HsmKeyType.SEED);
            String mac = hsmKey.mac(PLAIN_STRING_FOR_ENCRYPT, HsmMechanism.SEED_MAC);
